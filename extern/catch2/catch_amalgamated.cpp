...
ReporterConfig::ReporterConfig(IConfig const *_fullConfig,
    ...
    : m_stream(CATCH_MOVE(_stream)), m_fullConfig(_fullConfig),
      m_colourMode(colourMode), m_customOptions(CATCH_MOVE(customOptions)) {}
...
ColourImpl::ColourGuard::ColourGuard(ColourGuard &&rhs) noexcept
    : m_colourImpl(rhs.m_colourImpl), m_code(rhs.m_code),
...
RedirectGuard::RedirectGuard(RedirectGuard &&rhs) noexcept
    : m_redirect(rhs.m_redirect), m_activate(rhs.m_activate),
      m_previouslyActive(rhs.m_previouslyActive), m_moved(false) {
  rhs.m_moved = true;
}
...
public:
  ...
  AssertionPrinter(std::ostream &_stream, AssertionStats const &_stats,
      ...
      : stream(_stream), result(_stats.assertionResult),
        ...
        printInfoMessages(_printInfoMessages), colourImpl(colourImpl_) {}
...
public:
  ...
  ConsoleAssertionPrinter(std::ostream &_stream, AssertionStats const &_stats,
      ...
      : stream(_stream), stats(_stats), result(_stats.assertionResult),
        colour(Colour::None), messages(_stats.infoMessages),
        colourImpl(colourImpl_), printInfoMessages(_printInfoMessages) {
    switch (result.getResultType()) {
    case ResultWas::Ok:
...
public:
  ...
  TapAssertionPrinter(std::ostream &_stream, AssertionStats const &_stats,
      ...
      : stream(_stream), result(_stats.assertionResult),
        messages(_stats.infoMessages), itMessage(_stats.infoMessages.begin()),
        printInfoMessages(true), counter(_counter), colourImpl(colour_) {}
